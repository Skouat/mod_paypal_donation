	...
	if ($donate->verified)
	{
		...
		$sql = 'SELECT SUM(dd.net_amount) as amount
			...
			WHERE dd.confirmed = 1
				AND dd.payment_status = 'Completed'";
		$result = $db->sql_query($sql);
		set_config('donation_raised', (float) $db->sql_fetchfield('amount'), true);
...
switch ($mode)
{
	...
	default:
		...
		$s_hidden_fields = build_hidden_fields(array(
			...
			'no_shipping'	=> 1,
			'return'		=> append_sid($donate->page, 'mode=success'),
			'notify_url'	=> append_sid($donate->page),
			'cancel_return'	=> append_sid($donate->page, 'mode=cancel'),
		...
		$template->assign_vars(array(
			...
			'S_DONATE_FORMS'				=> $donate->u_paypal,
			'S_DONATION_DROPBOX'			=> (!empty($list_donation_value)) ? true : false,
			'S_SANDBOX'						=> $donate->use_sandbox,
			));
